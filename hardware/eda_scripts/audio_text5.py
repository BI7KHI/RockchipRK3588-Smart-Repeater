...
BLOCK = ("音频隔离衰减 · 设计说明\n"
         ...
         "    绕组间耐压 1000Vrms，额定电平约 1Vrms\n"
         "2) 变压器绕组不能过直流 → C1~C4 为串联隔直电容：\n"
         "    电台 MIC 有驻极体偏置，ELF2 MIC 口有 MICBIAS\n"
         "3) 衰减网络放在变压器初级之前，磁芯工作在小信号区，避免饱和\n"
         "4) 分压后源阻抗约 100~200Ω，接近 600Ω 设计值，低频响应好\n"
         "5) DEV GND 与 RPT GND 仅通过 U1/U2 磁耦合隔离\n"
         "    （PTT / RS485 等其它连线也必须隔离）\n"
         "6) 电台 SPK 若为 BTL 桥式输出（扩展口 1/16 脚均不接地）\n"
         "    → 本页单端接法不可用，须改为差分接入\n"
         "7) 音频走线远离 PTT / 射频走线，避免串扰")
NOTES = [
    (700, 560, 12.5, False, BLOCK),
