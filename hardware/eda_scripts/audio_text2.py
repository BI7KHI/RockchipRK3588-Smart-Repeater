...
r = ea.js("(async()=>{const R=window._EXTAPI_ROOT_;try{await R.sch_PrimitiveText.create(110,150,%s,0,%s,%s,11,false,false,false,2);"
          "return JSON.stringify({ok:true});}catch(e){return JSON.stringify({ok:false,err:String(e&&e.message||e)});}})()"
          % json.dumps(BLOCK, ensure_ascii=False), t=40)
print("new block ->", r)
